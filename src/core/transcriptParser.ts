...
export function processTranscriptLine(
  ...
  // Only JSON parsing is allowed to fail silently — a malformed/truncated
  // line is expected during writes. Anything the parsed record triggers
  // below is real logic and must not be swallowed as "malformed".
  ...
  if (record.type === 'assistant') {
    ...
    if (hasToolUse) {
      ...
      if (hasNonExemptTool) {
        startPermissionTimer(ctx, agentId, PERMISSION_EXEMPT_TOOLS);
      }
      ...
      startWaitingTimer(ctx, agentId, TEXT_IDLE_DELAY_MS);
    }
  } else if (record.type === 'progress') {
    processProgressRecord(ctx, agentId, record);
      ...
      if (hasToolResult) {
        for (const block of blocks) {
          if (block.type === 'tool_result' && block.tool_use_id) {
            ...
            setTimeout(() => {
              if (!ctx.agents.has(agentId)) return;
              ctx.send({ type: 'agentToolDone', id: agentId, toolId });
            }, TOOL_DONE_DELAY_MS);
          }
        }
  ...
  if (msgType === 'assistant') {
    ...
    for (const block of content) {
      if (block.type === 'tool_result' && block.tool_use_id) {
        ...
        setTimeout(() => {
          if (!ctx.agents.has(agentId)) return;
          ctx.send({ type: 'subagentToolDone', id: agentId, parentToolId, toolId });
        }, TOOL_DONE_DELAY_MS);
      }
    }
